...
def get_wallet_labels():
    labels = {}
    labels_str = os.environ.get("WALLET_LABELS", "")
    if labels_str:
        for item in labels_str.split(","):
            if "=" in item:
                addr, name = item.split("=", 1)
                labels[addr.strip()] = name.strip()
    return labels

VOLUME_ALERTED = set()
RSI_ALERTED = set()
WHALE_LAST_DATA = {}
WHALE_KNOWN_POSITIONS = {}
IS_FIRST_RUN = True
...
async def check_whales():
    global IS_FIRST_RUN, WHALE_KNOWN_POSITIONS, WHALE_LAST_DATA
    if not WHALE_WALLETS:
        return
    wallet_list = [w.strip() for w in WHALE_WALLETS.split(",") if w.strip()]
    wallet_labels = get_wallet_labels()
        ...
        try:
            ...
            for pos in positions:
                ...
                if notional < WHALE_MIN_SIZE:
                    continue
                current_positions[coin] = {"size": size, "entry": entry, "notional": notional, "pos": p}

            if IS_FIRST_RUN:
                WHALE_KNOWN_POSITIONS[address] = current_positions
                WHALE_LAST_DATA[address] = {k: {"size": v["size"], "entry": v["entry"]} for k, v in current_positions.items()}
                continue

            known = WHALE_KNOWN_POSITIONS.get(address, {})
            last = WHALE_LAST_DATA.get(address, {})

            for coin, data in current_positions.items():
                p = data["pos"]
                size = data["size"]
                entry = data["entry"]
                ...
                if coin not in known:
                    change = "New Position"
                elif abs(size) > abs(last.get(coin, {}).get("size", 0)) * 1.05:
                    change = "Increased"
                elif abs(size) < abs(last.get(coin, {}).get("size", 0)) * 0.95:
                    change = "Decreased"
                else:
                    continue
...
async def main():
    ...
    await check_whales()
    IS_FIRST_RUN = False
    print("First run complete - now tracking changes only!")
